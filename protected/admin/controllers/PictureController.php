...
class PictureController extends Controller
{
	...
	public function actionIndex()
	{
    	...
    	if(!empty($_GET['cid'])){
			...
			Category::model()->getAllCategoryIds($categoryList,Category::model()->findAll('module='.Yii::app()->params['module']['picture']),$_GET['cid']);
		...
		$this->render('index',array(
			...
			'categorys'=>Category::model()->showAllSelectCategory(Yii::app()->params['module']['picture'],Category::SHOW_ALLCATGORY),
		));
	}
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Picture']))
		{
			...
			if(!empty($upload))
			{
				$model->imgurl=Upload::createFile($upload,'picture','create');
			}
			if($model->save()){
		...
		$this->render('create',array(
			'model'=>$model,
			'categorys'=>Category::model()->showAllSelectCategory(Yii::app()->params['module']['picture']),
		));
	}
	...
	public function actionUpdate($id)
	{
		...
		if(!empty($_POST['Picture']))
		{
			...
			if(!empty($upload))
			{
				$model->imgurl=Upload::createFile($upload,'picture','update',$model->imgurl);
			}
			if($model->save()){
	...
	{
		$model=Picture::model()->findByPk((int)$id);
		if($model===null)
