...
class ProductController extends Controller
{
	...
	public function actionIndex()
	{
    	...
    	if(!empty($_GET['cid'])){
			...
			Category::model()->getAllCategoryIds($categoryList,Category::model()->findAll('module='.Yii::app()->params['module']['product']),$_GET['cid']);
		...
		$this->render('index',array(
			...
			'categorys'=>Category::model()->showAllSelectCategory(Yii::app()->params['module']['product'],Category::SHOW_ALLCATGORY),
		));
	}
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Product']))
		{
			...
			if(!empty($upload))
			{
				$model->imgurl=Upload::createFile($upload,'product','create');
			}
			if($model->save()){
		...
		$this->render('create',array(
			'model'=>$model,
			'categorys'=>Category::model()->showAllSelectCategory(Yii::app()->params['module']['product']),
			'bcategorys'=>Category::model()->showAllSelectCategory(Yii::app()->params['module']['brand']),
		));
	}
	...
	public function actionUpdate($id)
	{
		...
		if(!empty($_POST['Product']))
		{
			...
			if(!empty($upload))
			{
				$model->imgurl=Upload::createFile($upload,'product','update',$model->imgurl);
			}
			if($model->save()){
